  ...
  grunt.initConfig({
      ...
      livereload: {
        options: {
          ...
          middleware: function (connect) {
            return [
              modRewrite([
                '!\\.htm*|\\.md|\\.js|\\.css|\\.svg|\\.ico|\\.png|\\.eot|\\.otf|\\.ttf|\\.woff$ /index.html [L]'
              ]),
              connect.static('.tmp'),
              connect().use(
                '/bower_components',
    ...
    usemin: {
      ...
      options: {
        ...
        patterns: {
          js: [
            [/(images\/[^''""]*\.(png|jpg|jpeg|gif|webp))/g, 'Replacing references to images'],
            [/(svg\/[^''""]*\.(svg))/g, 'Replacing references to svg']
          ]
        }
      }
    },
